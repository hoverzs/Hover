...
ANECDOTES_SOURCE_CODE = "PG_BOOK_OF_300_ANECDOTES"
...
def test_import_inserts_all_parsed_anecdotes_as_draft(tmp_path: Path) -> None:
    conn = _fresh_connection(tmp_path)
    report = import_book_of_300_anecdotes(conn, raw_text_path=_require(ANECDOTES_SOURCE))
    conn.commit()

    assert report.source_code == ANECDOTES_SOURCE_CODE
    assert report.parsed_count == 345
    assert report.inserted_count == 345
...
def test_import_is_idempotent_no_duplicates_on_rerun(tmp_path: Path) -> None:
    conn = _fresh_connection(tmp_path)
    source_path = _require(ANECDOTES_SOURCE)

    first = import_book_of_300_anecdotes(conn, raw_text_path=source_path)
    conn.commit()
    second = import_book_of_300_anecdotes(conn, raw_text_path=source_path)
    conn.commit()

    assert first.inserted_count == 345
    assert second.inserted_count == 0
    assert second.skipped_existing_count == 345
    ...
    source_count = conn.execute(
        "SELECT COUNT(*) FROM sources WHERE code = ?", (ANECDOTES_SOURCE_CODE,)
    ).fetchone()[0]
    conn.close()
    assert count == 345
...
def test_import_records_raw_file_checksum_in_import_meta(tmp_path: Path) -> None:
    conn = _fresh_connection(tmp_path)
    source_path = _require(ANECDOTES_SOURCE)
    report = import_book_of_300_anecdotes(conn, raw_text_path=source_path)
    conn.commit()

    expected_sha256 = hashlib.sha256(source_path.read_bytes()).hexdigest()
    stored = conn.execute(
        "SELECT value FROM import_meta WHERE key = ?",
        (f"{ANECDOTES_SOURCE_CODE}.raw_file_sha256",),
    ).fetchone()[0]
    conn.close()
...
def test_published_without_hungarian_layer_still_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="content-completeness gate"):
        insert_story(
            ...
            external_ref="MISC/999",
            canonical_key="999",
            title_original="Some Other Anecdote",
...
def test_published_stories_view_stays_empty_for_freshly_imported_anecdotes(
    tmp_path: Path,
) -> None:
    conn = _fresh_connection(tmp_path)
    import_book_of_300_anecdotes(conn, raw_text_path=_require(ANECDOTES_SOURCE))
    conn.commit()
...
def test_anecdotes_and_other_sources_coexist_without_key_collisions(tmp_path: Path) -> None:
    ...
    if not baldwin_source.exists() or not jataka_source.exists():
        pytest.skip("Not all sibling raw sources present locally")

    conn = _fresh_connection(tmp_path)
    anecdotes = import_book_of_300_anecdotes(conn, raw_text_path=_require(ANECDOTES_SOURCE))
    ...
    conn.close()
    assert len({anecdotes.source_id, baldwin.source_id, jataka.source_id}) == 3
    assert total == 345 + 50 + 18
