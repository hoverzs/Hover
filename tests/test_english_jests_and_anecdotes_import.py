...
JESTS_SOURCE_CODE = "PG_ENGLISH_JESTS_AND_ANECDOTES"
...
def test_import_inserts_all_parsed_stories_as_draft(tmp_path: Path) -> None:
    ...
    report = import_english_jests_book(conn, raw_text_path=_require(JESTS_SOURCE))
    conn.commit()

    assert report.source_code == JESTS_SOURCE_CODE
    assert report.parsed_count == 758
    assert report.inserted_count == 758
...
def test_import_is_idempotent_no_duplicates_on_rerun(tmp_path: Path) -> None:
    conn = _fresh_connection(tmp_path)
    source_path = _require(JESTS_SOURCE)

    first = import_english_jests_book(conn, raw_text_path=source_path)
    conn.commit()
    second = import_english_jests_book(conn, raw_text_path=source_path)
    conn.commit()

    assert first.inserted_count == 758
    assert second.inserted_count == 0
    assert second.skipped_existing_count == 758
    ...
    source_count = conn.execute(
        "SELECT COUNT(*) FROM sources WHERE code = ?", (JESTS_SOURCE_CODE,)
    ).fetchone()[0]
    conn.close()
    assert count == 758
...
def test_published_stories_view_stays_empty_for_freshly_imported_stories(
    ...
    conn = _fresh_connection(tmp_path)
    import_english_jests_book(conn, raw_text_path=_require(JESTS_SOURCE))
    conn.commit()
...
def test_english_jests_and_other_sources_coexist_without_key_collisions(tmp_path: Path) -> None:
    ...
    conn = _fresh_connection(tmp_path)
    jests = import_english_jests_book(conn, raw_text_path=_require(JESTS_SOURCE))
    ...
    conn.close()
    assert len({jests.source_id, baldwin.source_id, anecdotes.source_id}) == 3
    assert total == 758 + 50 + 345
