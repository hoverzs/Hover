...
def test_canonical_self_tests_pass() -> None:
    ...
    assert all(r.ok for r in results if r.input != "__cross_input_consistency__")
    consistency = next(r for r in results if r.input == "__cross_input_consistency__")
    assert consistency.ok is True
    assert consistency.canonical == "John.4.1-42"
