"""Generic idempotent import glue, shared by every Project-Gutenberg-
sourced book parser (Jataka, Aesop, ...).

Extracted after implementing the second real source (Aesop) revealed
that the import logic — ensure the `sources` row exists, insert each
still-missing `stories` row as `status="draft"` with no Hungarian layer,
record raw-file + per-story checksums in `import_meta` — was byte-for-
byte identical to `jataka_importer.py`'s, differing only in which parser
function produced the `(canonical_key, external_ref, title_original,
original_text)` tuples. This was NOT planned as a generic framework in
advance; each book's own parser module still owns 100% of how its raw
text is delimited into stories.
"""
...
def import_pg_book(
    ...
    *,
    ...
    """Import one PG-sourced book (source row + its stories) idempotently.
    ...
    separate AI-enrichment phase's job, not this importer's.
    """
