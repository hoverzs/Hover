# Emmaus · digitális homiletikai műhely

**Verzió:** 1.0 (lásd `APP_VERSION` az `app.py` elején)

Református lelkipásztori környezetre szabott Streamlit-alkalmazás: bibliai szakasz alapján exegetikai, teológiai és homiletikai segédleteket készít a **Google Gemini** API-val. A projekt **ingyenes és nyílt forráskódú** ([MIT licenc](LICENSE)).
...
- Emmaus-arculat: háttér, glassmorphism, tipográfia (részletek az `app.py` CSS-blokkjában)
...
- **Ikonok / logó:** az `app.py` a `icons/` mappa és egyéb útvonalak alapján keres képeket (pl. `igehely.png`). Ha nincs fájl, van elegáns fallback.
...
*„Nem hevült-e a szívünk…” — Lk 24,32*
