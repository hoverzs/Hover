"""Phase 3I: user-facing illustration retrieval UI.
...
THIS module is the normal, public "Illusztrációk" tab feature, added
ALONGSIDE (not replacing) the existing free-form LLM-brainstorm action
already there. Its own access boundary is not about WHO the user is --
...
def render_illustration_search_action(*, generate_fn: Callable[..., str] | None = None) -> None:
    """Renders the "Illusztrációk keresése" action -- call this from
    `app.py` inside the existing "Illusztrációk" tab, AFTER the existing
    `render_section_tab(key="illustrations", ...)` call, never in place
    of it."""
    st.divider()
    st.markdown("##### Illusztrációk keresése (ellenőrzött adatbázisból)")
    st.caption(
        "A meglévő textus alapján keres a Textus saját, ember és gép által "
        "ellenőrzött illusztráció-adatbázisában — a fenti szabad ötletlistától "
        "függetlenül."
    )
    ...
    if st.button("Illusztrációk keresése", key="ill_retrieval_search_btn"):
        if not passage:
